...
class UserInviteCodeTest {
    ...
    @Test
    void profileUpdateKeepsUsernameAndInviteCodeImmutable() {
        User user = new User();
        user.setUsername(USERNAME);
        user.setPassword("password123");
        ...
        assertThat(userService.updateProfile(saved.getId(), "新的昵称", "new@example.com", "13800000000")).isTrue();

        User updated = userService.getById(saved.getId());
        assertThat(updated.getUsername()).isEqualTo(USERNAME);
        assertThat(updated.getInviteCode()).isEqualTo(inviteCode);
        assertThat(updated.getNickname()).isEqualTo("新的昵称");
